...
class FirebaseService {
  ...
  Stream<List<Shoe>> streamShoes() {
    ...
    final collectionRef = _firestore.collection('users').doc(_userId).collection('shoes');
    ...
    return collectionRef.snapshots().map((snapshot) {
      ...
      return snapshot.docs.map((doc) {
        ...
        return shoe.copyWith(
          itemId: itemIdFromDocId,
        );
      }).toList();
    });
  }
  ...
  Future<String?> uploadImage(File file, int shoeId) async {
    try {
      ...
      final uploadPath = 'shoes/$_userId/${shoeId}_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<void> saveShoe(Shoe shoe, {File? localImageFile}) async {
    ...
    final dataToSave = shoe.copyWith(
    ...
    debugPrint('Shoe data saved successfully for Item ID: ${shoe.itemId} (Doc ID: $docId)');
  }
  ...
  Future<void> deleteShoe(Shoe shoe) async {
    ...
    debugPrint('Shoe document deleted successfully for Item ID: ${shoe.itemId}');
  }
